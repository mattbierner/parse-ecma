...
define(["require", "exports", "bennu/parse", "bennu/lang", "bennu/text", "nu-stream/stream", "./line_terminator_lexer",
    "./number_lexer"
], (function(require, exports, parse, parse_lang, parse_text, stream, line_terminator_lexer, number_lexer) {
    "use strict";
    var doubleQuote, escape, singleQuote, lineContinuation, singleEscapeCharacter, escapeCharacter,
            ...
            doubleStringCharacter, doubleStringCharacters, doubleStringLiteral, stringLiteral, __add = (
                function(x, y) {
                    return (x + y);
                }),
        join = __add;
    ...
    (singleEscapeCharacter = parse.choice(parse_text.character("'"), parse_text.character("\""), parse_text.character(
        "\\"), parse.next(parse_text.character("b"), parse.always("\b")), parse.next(parse_text.character(
        "f"), parse.always("\f")), parse.next(parse_text.character("n"), parse.always("\n")), parse.next(
        parse_text.character("r"), parse.always("\r")), parse.next(parse_text.character("t"), parse.always(
        "\t")), parse.next(parse_text.character("v"), parse.always("\u000b"))));
    (escapeCharacter = parse.choice(singleEscapeCharacter, number_lexer.decimalDigit, parse_text.character("u"),
        parse_text.character("x")));
    (nonEscapeCharacter = parse.token((function(tok) {
        return (!(parse.test(escapeCharacter, tok) || parse.test(line_terminator_lexer.lineTerminator,
            tok)));
    })));
    ...
    (escapeSequence = parse.choice(characterEscapeSequence, parse.next(parse_text.character("0"), parse.next(
        parse.either(parse.eof, parse.token((function(tok) {
            return (!parse.test(number_lexer.decimalDigit, tok));
        }))), parse.always("\u0000"))), hexEscapeSequence, unicodeEscapeSequence));
    (singleStringCharacter = parse.choice(parse.attempt(lineContinuation), parse.next(escape, escapeSequence),
    ...
    (singleStringLiteral = parse.Parser("Single String Literal", parse_lang.between(singleQuote, singleQuote,
        parse.bind(singleStringCharacters, (function(str) {
            return parse.always(stream.foldl(join, "", str));
        })))));
    (doubleStringCharacter = parse.choice(parse.attempt(lineContinuation), parse.next(escape, escapeSequence),
        parse.token((function(tok) {
    ...
    (doubleStringLiteral = parse.Parser("Double String Literal", parse_lang.between(doubleQuote, doubleQuote,
