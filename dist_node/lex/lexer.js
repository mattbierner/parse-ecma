...
var parse = require("bennu")["parse"],
    ...
    lexer, lexStream, lex, always = parse["always"],
    ...
    type, type0, type1, type2, type3, p, type4, type5, type6, type7, p0, type8, p1, type9, p2, consume = (function(tok,
        self) {
        switch (tok.type) {
            case "Comment":
            case "Whitespace":
    ...
    literal = choice(((type = lexToken.StringToken.create), stringLiteral.map((function(x) {
        ...
        return [type2, x];
    }))), ((type3 = lexToken.RegularExpressionToken.create), (p = next(isRegExpCtx, regularExpressionLiteral)), p.map(
        (function(x) {
            return [type3, x];
        })))),
    ...
    inputElement = choice(((type7 = lexToken.CommentToken), (p0 = comment_lexer.comment), p0.map((function(x) {
        return [type7, x];
    }))), ((type8 = lexToken.WhitespaceToken), (p1 = whitespace_lexer.whitespace), p1.map((function(x) {
        return [type8, x];
    }))), ((type9 = lexToken.LineTerminatorToken), (p2 = line_terminator_lexer.lineTerminator), p2.map((function(x) {
        return [type9, x];
    }))), token);
