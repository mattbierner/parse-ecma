...
(arrayElement = parse.Parser("Array Element", parse.either(assignmentExpression, parse.next(parse.look(token.punctuator(
    ",")), parse.always(null)))));
(arrayElements = parse.Parser("Array Elements", parse.eager(parse_lang.sepEndBy(token.punctuator(","), arrayElement))));
(arrayLiteral = parse.Parser("Array Literal", ecma_parse.node(parse_lang.between(token.punctuator("["), token.punctuator(
    "]"), arrayElements), ast_expression.ArrayExpression.create)));
(propertyName = parse.Parser("Property Name", parse.expected("property name", parse.choice(value.identifier, value.stringLiteral,
...
var valueProperty = parse.Parser("Value Property", ecma_parse.nodea(parse.enumeration(parse.attempt(propertyValueKey),
    parse.expected("Assignment Expression", propertyValueInitializer)), ast_value.ObjectValue.create)),
    propertyGetterKey = parse.Parser("Property Getter Key", parse.next(token.identifier("get"), propertyName));
(propertyGetInitializer = parse.Parser("Property Get Initializer", ecma_parse.node(parse.next(parse.next(token.punctuator(
...
var getterProperty = parse.Parser("Getter Property", ecma_parse.nodea(parse.enumeration(propertyGetterKey,
    ...
    propertySetterKey = parse.Parser("Property Setter Key", parse.next(token.identifier("set"), propertyName));
(propertySetInitializer = parse.Parser("Property Set Initializer", ecma_parse.nodea(parse.enumeration(parse_lang.between(
...
(propertyInitializer = parse.Parser("Property Initializer", parse.choice(valueProperty, getterProperty, setterProperty)));
(propertyList = parse.Parser("Property List", parse.eager(parse_lang.sepEndBy(token.punctuator(","),
    propertyInitializer))));
(objectLiteral = parse.Parser("Object Literal", ecma_parse.node(parse_lang.between(token.punctuator("{"), token.punctuator(
    "}"), propertyList), ast_expression.ObjectExpression.create)));
(thisExpression = parse.Parser("This Expression", ecma_parse.node(token.keyword("this"), ast_expression.ThisExpression.create)));
(primaryExpression = parse.memo(parse.Parser("Primary Expression", parse.choice(functionExpression, thisExpression,
...
(bracketAccessor = parse.Parser("Bracket Accessor", ecma_parse.node(parse_lang.between(token.punctuator("["), token.punctuator(
...
(newExpression = parse.memo(parse.Parser("New Expression", ecma_parse.nodea(parse.next(token.keyword("new"), parse.enumeration(
...
(postfixExpression = parse.Parser("Postfix Expression", ecma_parse.nodea(parse.enumeration(leftHandSideExpression,
    parse.optional(null, postfixOperator)), (function(loc, argument, op) {
...
(unaryOperator = parse.Parser("Unary Operator", parse.either(token.keyword("delete", "void", "typeof"), token.punctuator(
    "++", "--", "+", "-", "~", "!"))));
...
(unaryExpression = parse.Parser("Unary Expression", parse.binds(parse.enumeration(parse.many(unaryOperator), parse.expected(
...
(binaryExpression = parse.Parser("Binary Expression", ecma_parse.precedence(unaryExpression, precedenceTable)));
(binaryExpressionNoIn = parse.Parser("Binary Expression", ecma_parse.precedence(unaryExpression, precedenceTableNoIn)));
var binExpr, assignExpr;
(conditionalExpression = parse.Parser("Conditional Expression", ((binExpr = binaryExpression), (assignExpr = parse.expected(
...
(assignmentOperator = parse.Parser("Assignment Operator", token.punctuator("=", "*=", "*=", "/=", "%=", "+=", "-=",
    "<<=", ">>=", ">>>=", "&=", "^=", "|=")));
var condExpr;
(assignmentExpression = parse.Parser("Assignment Expression", ((condExpr = conditionalExpression), parse.rec((function(
    self) {
...
var condExpr0;
(assignmentExpressionNoIn = parse.Parser("Assignment Expression No In", ((condExpr0 = conditionalExpressionNoIn), parse
    .rec((function(self) {
        return parse.either(parse.binds(parse.attempt(parse.enumeration(leftHandSideExpression,
            assignmentOperator)), (function(left, operator) {
            return parse.bind(parse.expected("assignment expression", self), (function(
...
var expr;
(expression = parse.Parser("Expression", ((expr = parse.expected("expression", assignmentExpression)), ecma_parse.node(
...
(expressionNoIn = parse.Parser("Expression No In", ((expr0 = parse.expected("expression no in",
    assignmentExpressionNoIn)), ecma_parse.node(parse.eager(parse_lang.sepBy1(token.punctuator(","), expr0)), (
    function(loc, list) {
        return ((list.length > 1) ? new(ast_expression.SequenceExpression)(loc, list) : list[0]);
    })))));
