...
var tokenizer = function(token) {
    ...
    return parse.rec(function(self){
        return parse.choice(
            ...
            parse.bind(
                parse.expected("comment", lexer.comment),
                function(x){
                    return (x.value.indexOf('\n') !== -1 ?
                        parse.bind(parse.next(parse.many(lexer.lineTerminator), self), followLineTerminator) :
                        self);
                }),
            parse.next(
            ...
            parse.expected("line terminator",
                parse.bind(parse.next(parse.many1(lexer.lineTerminator), self), followLineTerminator)),
            token);
    });
};
...
var createNextDivState = function(input, loc, ok, err) {
    return parse.perform(
        ...
        function(x, state) {
            return ok(new ParserState(input, state.position, x, state.input, false, x === null, loc.end));
        },
        function(x, state) {
            return err(new ParserState(input, state.position, x, state.input, false, true, loc.end));
        });
};
 ...
 * 
 */
var ParserState = function(input, pos, first, rest, error, isEmpty, prevEnd) {
