...
var emptyStatement = parse.Parser('Empty Statement',
    ecma_parse.node(
        token.punctuator(';'),
        function(loc) {
            return new ast_statement.EmptyStatement(loc);
        }));
...
var expressionStatement = parse.Parser('Expression Statement',
    ecma_parse.node(
        ...
        function(loc, expression) {
            return new ast_statement.ExpressionStatement(loc, expression);
        }));
...
var ifStatement = parse.Parser('If Statement',
    ecma_parse.nodea(
        ...
        function(loc, test, consequent, alternate) {
            return new ast_statement.IfStatement(loc, test, consequent, alternate);
        }));
...
var whileStatement = parse.Parser('While Statement',
    ecma_parse.nodea(
        ...
        function(loc, test, body) {
            return new ast_statement.WhileStatement(loc, test, body);
        }));
...
var forInit = parse.either(
    ecma_parse.node(
        ...
        function(loc, declarations) {
            return new ast_declaration.VariableDeclaration(loc, declarations);
        }),
...
var forInLeft = parse.either(
    ecma_parse.node(
        ...
        function(loc, x) {
            return new ast_declaration.VariableDeclaration(loc, [x]);
        }),
...
var forInStatement = parse.Parser('For In Statement',
    ecma_parse.nodea(
        ...
        function(loc, left, right, body) {
            return new ast_statement.ForInStatement(loc, left, right, body);
        }));
...
var continueStatement = parse.Parser('Continue Statement',
    ecma_parse.node(
        ...
        function(loc, label) {
            return new ast_statement.ContinueStatement(loc, label);
        }));
...
var breakStatement = parse.Parser('Break Statement',
    ecma_parse.node(
        parse_lang.between(token.keyword('break'), logicalSemiColon,
            parse.optional(null, noLineTerminator(value.identifier))),
        function(loc, label) {
            return new ast_statement.BreakStatement(loc, label);
        }));
...
var returnStatement = parse.Parser('Return Statement',
    ecma_parse.node(
        ...
        function(loc, argument) {
            return new ast_statement.ReturnStatement(loc, argument);
        }));
...
var withStatement = parse.Parser('With Statement',
    ecma_parse.nodea(
        ...
        function(loc, object, body) {
            return new ast_statement.WithStatement(loc, object, body);
        }));
...
var labeledStatement = parse.Parser('Labeled Statement',
    ecma_parse.nodea(
        ...
        function(loc, label, body) {
            return new ast_statement.LabeledStatement(loc, label, body);
        }));
...
var switchStatement = parse.Parser('Switch Statement',
    ecma_parse.nodea(
        ...
        function(loc, discriminant, cases) {
            return new ast_statement.SwitchStatement(loc, discriminant, cases);
        }));
...
var throwStatement = parse.Parser('Throw Statement',
    ecma_parse.node(
        ...
        function(loc, argument) {
            return new ast_statement.ThrowStatement(loc, argument);
        }));
...
var tryStatement = parse.Parser('Try Statement',
    ecma_parse.nodea(
        ...
        function(loc, block, handler, finalizer) {
            return new ast_statement.TryStatement(loc, block, handler, finalizer);
        }));
...
var debuggerStatement = parse.Parser('Debugger Statement',
    ecma_parse.node(
        parse.next(
            ...
            token.punctuator(';')),
        function(loc) {
            return new ast_statement.DebuggerStatement(loc);
        }));
