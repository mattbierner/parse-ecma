...
define(['require',
        ...
        'ecma/ast/expression', 'ecma/ast/statement'],
function(require,
        parse, parse_eager,
        stream,
        position,
    ...
    return parse.bind(
        ...
        function(x) {
                ...
                if (tok.type) {
                    ...
                    while (stack.length > 0) {
                        ...
                        if (o2.precedence <= tok.precedence) {
                            ...
                            var rt = out.pop(),
                                lf = out.pop();
                            var node = new astExpression[o2.node](o2.value, lf, rt);
                            node.loc = new position.SourceLocation(lf.loc.start, rt.loc.end);
                            out.push(node);
                        } else {
                            break;
                        }
            ...
            while (stack.length > 0){
                ...
                var rt = out.pop(),
                    lf = out.pop();
                var node = new astExpression[o.node](o.value, lf, rt);
                node.loc = new position.SourceLocation(lf.loc.start, rt.loc.end);
                out.push(node);
            }
...
// Array Literal
////////////////////////////////////////
var elementList = ecma_parse.sepEndBy(token.punctuator(','),
    parse.either(
...
// Object Literal
////////////////////////////////////////
var propertyName = parse.expected('property name', parse.choice(
    value.identifier,
    value.stringLiteral,
...
var propertyGetInitilizer = parse.next(
...
var propertySetInitilizer = parse.next(
...
var propertyAssignment = parse.choice(
    ...
    propertyValueInitilizer);

var propertyNameAndValueList = parse.bind(
    ecma_parse.sepEndBy1(token.punctuator(','),
        propertyAssignment),
    function(props){
        return parse.always(stream.toArray(props));
    });

var objectLiteralProperties = parse.either(
...
var args = ecma_parse.astNode(parse.between(token.punctuator('('), token.punctuator(')'),
    argumentList));

var dotAccessor = ecma_parse.astNode(parse.bind(
    parse.next(
        token.punctuator('.'),
        value.identifier),
    function(x) {
...
var bracketAccessor = ecma_parse.astNode(parse.bind(
    parse.between(token.punctuator('['), token.punctuator(']'),
...
var memberExpression = (function(){
    var reducer = function(p, c){
        var node = new astExpression.MemberExpression(p, c.property, c.computed);
        node.loc = new position.SourceLocation(p.loc.start, c.loc.end);
        return node;
    };
 ...
 * 
 */
var newExpression = ecma_parse.astNode(parse.next(
    token.keyword('new'),
    parse.binda(
        parse.sequence(
...
(function(){
    var reducer = function(p, c) {
        var node = (c.hasOwnProperty('property') ?
...
var postfixExpression = parse.binda(
    ...
    function(argument, op) {
        if (!op) {
            return parse.always(argument);
        }
        var node = new astExpression.UpdateExpression(op.value, argument, false);
        node.loc = new position.SourceLocation(argument.loc.start, op.loc.end);
