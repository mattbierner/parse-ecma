...
var arrayLiteral = parse.Parser('Array Literal',
    ecma_parse.node(
        ...
        function(loc, elements) {
            return new ast_expression.ArrayExpression(loc, elements);
        }));
    ...
    ecma_parse.nodea(
        parse.next(
            ...
            parse.sequence(
                ...
                parse_lang.between(token.punctuator('{'), token.punctuator('}'),
                    functionBody))),
        function(loc, name, _0, _1, body) {
            return {
                'loc': loc,
                'key': name,
                'kind': 'get',
                'value': new ast_expression.FunctionExpression(loc, null, [], body)
            };
        }));
    ...
    ecma_parse.nodea(
        parse.next(
            ...
            parse.sequence(
                ...
                parse_lang.between(token.punctuator('{'), token.punctuator('}'),
                    functionBody))),
        function(loc, name, parameters, body) {
            return {
                'loc': loc,
                'key': name,
                'kind': 'set',
                'value': new ast_expression.FunctionExpression(loc, null, parameters, body)
            };
        }));
...
var objectLiteral = parse.Parser('Object Literal',
    ecma_parse.node(
        ...
        function(loc, properties) {
            return new ast_expression.ObjectExpression(loc, properties);
        }));

// This Expression
////////////////////////////////////////
/**
 * Parser for a this expression.
 */
var thisExpression = parse.Parser('This Expression',
    ecma_parse.node(
        token.keyword('this'),
        function(loc) {
            return new ast_expression.ThisExpression(loc);
        }));
...
var memberExpression = (function(){
    var reducer = function(p, c){
        return new ast_expression.MemberExpression(
    ...
    ecma_parse.nodea(
        parse.next(
            ...
            parse.sequence(
                ...
                parse.optional([], args))),
        function(loc, expression, a) {
            return new ast_expression.NewExpression(loc, expression, a);
        }));
...
var leftHandSideExpression = (function(){
    var reducer = function(p, c) {
        ...
        return (c.hasOwnProperty('property') ?
             new ast_expression.MemberExpression(loc, p, c.property, c.computed) :
             new ast_expression.CallExpression(loc, p, c));
    };
...
var postfixExpression = parse.Parser('Postfix Expression',
    ecma_parse.nodea(
        ...
        function(loc, argument, op) {
            return (!op ?
                argument :
                new ast_expression.UpdateExpression(
    ...
    return parse.backtrack(parse.either(
        parse.attempt(ecma_parse.nodea(
            parse.sequence(
                ...
                token.punctuator(':'),
                assignExpr),
            function(loc, bin, _1, consequent, _2, alternate) {
            ...
            parse.binds(
                ...
                function(left, operator) {
                    return parse.bind(
                        ...
                        function(right) {
                            return parse.always(new ast_expression.AssignmentExpression(
