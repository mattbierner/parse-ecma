 ...
 */
var arrayElement = parse.Parser('Array Elements',
    parse.either(
...
var arrayElements = parse.Parser('Array Elements',
    parse.bind(
        ecma_parse.sepEndBy(token.punctuator(','),
            arrayElement),
        function(elements) {
...
var memberExpression = (function(){
    var reducer = function(p, c){
        return new astExpression.MemberExpression(
...
var leftHandSideExpression = (function(){
    var reducer = function(p, c) {
        ...
        return (c.hasOwnProperty('property') ?
             new astExpression.MemberExpression(loc, p, c.property, c.computed) :
             new astExpression.CallExpression(loc, p, c));
    };
...
var postfixExpression = parse.Parser('Postfix Expression',
    ecma_parse.nodea(
        ...
        function(loc, argument, op) {
            return (!op ?
                argument :
                new astExpression.UpdateExpression(
...
var unaryExpression = (function(){
    var reducer = function(argument, op) {
        ...
        return (op.value === '++' || op.value === '--' ?
            new astExpression.UpdateExpression(loc, op.value, argument, true) :
            new astExpression.UnaryExpression(loc, op.value, argument));
    };
    ...
    return ecma_parse.node(
        ...
        function(loc, list) {
            return (list.length > 1 ?
                new astExpression.SequenceExpression(loc, list) :
                list[0]);
        });
};
