        ...
        parse.binda(
            ...
            function(name, _0, _1, body) {
                return parse.always({
                    'key': name,
                    'kind': 'get',
                    'value': new astExpression.FunctionExpression(
                        null,
                        [],
...
var _baseConditionalExpressionParser = function(binExpr, assignExpr){
    return parse.binda(
        parse.sequence(
            binExpr,
            parse.optional(parse.next(token.punctuator('?'),
                parse_eager.sequence(
                    assignExpr,
                    token.punctuator(':'),
                    assignExpr)))),
        function(test, conditional) {
            return parse.always(conditional ? 
                new astExpression.ConditionalExpression(test, conditional[0], conditional[2]) :
                test);
        });
};
