 ...
 */
var singleLineCommentMarker = parse_text.string('//');
 ...
 */
var multiLineCommentStartMarker = parse_text.string('/*');

/**
 * Lexer for token that marks the end of a multi line comment.
 */
var multiLineCommentEndMarker = parse_text.string('*/');
...
var multiLineCommentChars = parse.RecParser('Multi Line Comment Characters Lexer', function(self) {
    return parse.either(
        parse.next(
            ...
            parse.either(
                parse.next(parse_text.character('/'), parse.always(stream.end)),
